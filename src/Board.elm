...
type alias Board =
    ...
    , staticBlocks : List (Scene3d.Entity WorldCoordinates)
    , staticWallsMesh : Scene3d.Mesh.Uniform WorldCoordinates
    }
...
viewWalls : Scene3d.Mesh.Uniform WorldCoordinates -> Scene3d.Entity WorldCoordinates
viewWalls wallMesh =
    let
        mesh =
            Scene3d.Mesh.cullBackFaces wallMesh
    in
    Scene3d.meshWithShadow
...
viewBlockDynamic : ( Point, Block ) -> Scene3d.Entity WorldCoordinates
viewBlockDynamic ( point, block ) =
    let
        ( x, y, z ) =
            point
    in
...
optimize board =
    let
        blockList =
            Dict.toList board.blocks

        toTris p1 p2 p3 p4 =
            [ Triangle3d.from p1 p2 p3
            , Triangle3d.from p1 p3 p4
            ]
                    ...
                    (\( ( x, y, z ), block ) ->
                        case block of
                            Wall ->
                                ...
                                in
                                List.concat
                                    [ case Dict.get ( x + 1, y, z ) board.blocks of
                                        Just Wall ->
                                            []
                                        ...
                                        _ ->
                                            let
                                                p1 =
                                                    center
                                                        |> Point3d.translateIn Direction3d.positiveX (Length.meters 0.5)
                                                        |> Point3d.translateIn Direction3d.positiveY (Length.meters -0.5)
                                                        |> Point3d.translateIn Direction3d.positiveZ (Length.meters 0.5)

                                                p2 =
                                                    p1
                                                        |> Point3d.translateIn Direction3d.positiveZ (Length.meters -1.0)

                                                p3 =
                                                    p2
                                                        |> Point3d.translateIn Direction3d.positiveY (Length.meters 1.0)
                                    ...
                                    , case Dict.get ( x - 1, y, z ) board.blocks of
                                        ...
                                        _ ->
                                            ...
                                            in
                                            toTris p1 p2 p3 p4
                                    , case Dict.get ( x, y + 1, z ) board.blocks of
                                        Just Wall ->
                                            []
                                        ...
                                        _ ->
                                            let
                                                ...
                                                p3 =
                                                    p2
                                                        |> Point3d.translateIn Direction3d.positiveX (Length.meters -1.0)
                                    ...
                                    , case Dict.get ( x, y - 1, z ) board.blocks of
                                        ...
                                        _ ->
                                            ...
                                            in
                                            toTris p1 p2 p3 p4
                                    , case Dict.get ( x, y, z + 1 ) board.blocks of
                                        Just Wall ->
                                            []
    ...
    in
    { board
        | staticBlocks = singleBlocks
        , staticWallsMesh = walls
    }
...
viewGameOver level nextActions =
    if level.hearts > 0 then
        ...
        Html.div
            ...
            , Html.Attributes.style "backdrop-filter" "blur(20px)"
